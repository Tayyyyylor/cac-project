import "./Bio.css"

function Bio() {
  return (
    <article className="bio-container">
        <hgroup className="title-container">
        <h2 className="title-bio">De Coeur à Coeur</h2>
        <h3 className="subtitle-bio"> Conseil en langage verbal - Non verbal - Écrit </h3>
        <h6 className="name-bio"> PAR <span className='color-letter-capital'>F</span>RANCIS <span className='color-letter-capital'>D</span>ECKER</h6>
        </hgroup>
        <p className="text-bio">C'est d'une existence atypique, constituée de moments montagneux, tantôt planes, tantôt doux, ponctuée de rencontres indélébiles avec les Grands Singes, le monde tribal ( Le peuple Maasaï et le peuple Sioux Lakota ) que commença une partie de ma vie. Puis vinrent les expériences d'acteur, de coach en gestuelle sur des films et une plongée dans le monde du spiritueux avec mes premières expériences d'orateur dans des palaces, des casinos.  

Le fil rouge de tout ceci est la communication et la volonté de savoir qui est mon interlocuteur. Qui est-il ? Que ressent-il ? Dans quelle voie souhaite t-il aller ? Une communication que je souhaite la plus organique possible, dans le respect de chacun et s'adaptant à diverses situations. 

Ma famille a contribué à forger mon tempérament, j'y ai fait mes premières expériences de médiation, de recréation de lien en mettant en avant un langage issu du cœur. Mon entourage d'amis fit appel à mes capacités pour les préparer à des rendez-vous amoureux. Ces mêmes amis et relations familiales qui m'ont demandé de les préparer à des entretiens à des recrutements.

Plus que la réussite à ces moments décisifs, importe surtout le meilleur souvenir déposé auprès de celle ou celui qui écoutera. Un souvenir très agréable et d'une rare sincérité marquera durablement la mémoire et créera la connexion, un lien suffisamment important et rare 

Ainsi est né "De Coeur à Coeur " : Une démarche unique dont le but est la création ou la recréation d'un lien rare et important. Son logo est un leader indien d'Amérique du Nord, en réponse à mes influences. Le leader indien dont la présence même est de créer la connexion, l'entretenir afin de le guider.</p>
    <div className="video-bio"></div>
    </article>
  )
}

export default Bio